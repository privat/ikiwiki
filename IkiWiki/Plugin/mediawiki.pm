...
sub check_redirect {
	...
	if (! length $link) {
	    return '<b>Redirect Error:</b> <nowiki><span class="createlink"><a href="http://ikiwiki.info/ikiwiki.cgi?page=__36__redir_page&amp;from=tips%2Fconvert_mediawiki_to_ikiwiki%2Fdiscussion&amp;do=create" rel="nofollow">?</a>&#36;redir page</span> not found.</nowiki>';
      } # SKC note: changed double to single quotes
	...
	while (exists $pagestate{$at}{mediawiki}{redir}) {
	    if ($seen{$at}) {
		return '<b>Redirect Error:</b> cycle found on <nowiki><span class="createlink"><a href="http://ikiwiki.info/ikiwiki.cgi?page=__36__at&amp;from=tips%2Fconvert_mediawiki_to_ikiwiki%2Fdiscussion&amp;do=create" rel="nofollow">?</a>&#36;at</span></nowiki>';
	    } # SKC note: ditto
	    $seen{$at}=1;
